...
class TelegramMessageParser:
    ...
    def add_handlers(self):
        ...
        self.bot.add_handler(CommandHandler("dark", self.darkgpt))
    ...
    async def chat_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not allowed:
            await context.bot.send_message(
                ...
                text = "Вам запрещено срать своими запросами в этом боте!"
            )
            return
        ...
        if not allowed:
            results = [
                InlineQueryResultArticle(
                    id = str(uuid4()),
                    title = "Sorry😢",
                    description = "Вам запрещено срать своими запросами в этом боте!",
                    input_message_content = InputTextMessageContent("Вам запрещено срать своими запросами в этом боте!")
                )
            ]
        else:
            results = [
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="Привет! Я ChatGPT бот!"
        )
    
    async def darkgpt(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        message = ""
        with open("darkgpt.txt", "r") as file:
            message = file.read()
...
/chat - Позвать бота в групповом чате
/dalle - Использование: "/dalle <prompt>" Создать картинку.
/clear - Очистить контекст для разговора.
/dark - Включить режим DarkGPT(Подождите пару секунд)
/help - Помощь
